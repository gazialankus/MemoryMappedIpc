...
namespace MemoryMappedIpcServer.Shared {
    class SharedMemoryAccessor {
        ...
        private void SeekToLine(Stream baseStream, int where) {
        ...
        public void AddLine(MotionMessage m) {
            ...
            if (GetLinePos(_bufferWriter.BaseStream) == TotalBufferSizeInLines) {
                SeekToLine(_bufferWriter.BaseStream, 0);
        ...
        public IEnumerable<MotionMessage> GetAvailableLinesToClient() {
            ...
            if (ClientHasReadThisManyLines > 0) {
                ...
                IEnumerable<MotionMessage> enumerable = EnumerateForClient(bufferWrittenLineCountCopy, _bufferReader);
        ...
        private IEnumerable<MotionMessage> EnumerateForClient(int bufferWrittenLineCountCopy, BinaryReader bufferReader) {
            // read them, starting from BufferStartingLine, and as many as BufferWrittenLineCount
            // need to seek to BufferStartingLine
            SeekToLine(bufferReader.BaseStream, BufferStartingLine);
            ...
            for (int i = 0; i < bufferWrittenLineCountCopy; ++i) {
                ...
                if (GetLinePos(_bufferReader.BaseStream) == TotalBufferSizeInLines) {
                    SeekToLine(bufferReader.BaseStream, 0);
                }
